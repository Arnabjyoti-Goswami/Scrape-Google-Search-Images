...
def generate_google_images_url(query):
  ...
  query_string = '&'.join([f"{key}={value}" for key, value in params.items()]) # iterates through the key-value pairs in the params dictionary and creates a list of strings in the format "key=value" for each pair. query_string is constructed by joining the list of strings created in this step with the "&" character. This creates a query string that can be appended to the base URL.
  url = f"{base_url}?{query_string}"
  return url

def make_folder(query):
  '''
  Make a different folder for each query, if there are multiple queries
  '''
  folder_name = query
...
def download_image(url, dir_path, num):
  ...
  try:
    with urllib.request.urlopen(url, timeout=3) as response, open(filepath, 'wb') as f:
      f.write(response.read())
      print(f'Image {num} downloaded successfully! URL: ({url})')
  except urllib.error.URLError as e:
    if isinstance(e.reason, socket.timeout):
      print(f'Timeout of {download_timeout} seconds reached while trying to download image {num}, will move onto the next one')
    else:
...
def download_all_images(
  ...
  for i in range(1, num_images):
    ...
    preview_image_element = driver.find_element('xpath', preview_image_element_xpath)
    ...
    preview_image_element_container = driver.find_element('xpath', preview_image_element_container_xpath)
    ...
    while True:
      ...
      image_url = image_element.get_attribute('src') # this is the actual fully loaded image url after clicking on an element

      if image_url != preview_image_url: # if the urls became different, break the loop 
        break
        ...
        if current_time - start_time > timeout_time:
          if low_res_upon_timeout:
            print('Timeout! Will download the lower resolution image and move onto the next one')
            image_url = preview_image_url
          else:
            print('Timeout! Will not download the lower resolution image and move onto the next one')
          break
